...
function baseOption(e){return{parsers:[new parser.ENISOFormatParser(e),new parser.ENDeadlineFormatParser(e),new parser.ENMonthNameLittleEndianParser(e),new parser.ENMonthNameMiddleEndianParser(e),new parser.ENMonthNameParser(e),new parser.ENSlashDateFormatParser(e),new parser.ENSlashDateFormatStartWithYearParser(e),new parser.ENSlashMonthFormatParser(e),new parser.ENTimeAgoFormatParser(e),new parser.ENTimeExpressionParser(e),new parser.JPStandardParser(e),new parser.ESTimeAgoFormatParser(e),new parser.ESDeadlineFormatParser(e),new parser.ESTimeExpressionParser(e),new parser.ESMonthNameLittleEndianParser(e),new parser.ESSlashDateFormatParser(e),new parser.FRDeadlineFormatParser(e),new parser.FRMonthNameLittleEndianParser(e),new parser.FRSlashDateFormatParser(e),new parser.FRTimeAgoFormatParser(e),new parser.FRTimeExpressionParser(e),new parser.ZHHantDateParser(e),new parser.ZHHantWeekdayParser(e),new parser.ZHHantTimeExpressionParser(e),new parser.ZHHantCasualDateParser(e),new parser.ZHHantDeadlineFormatParser(e)],refiners:[new refiner.OverlapRemovalRefiner,new refiner.ForwardDateRefiner,new refiner.ENMergeDateTimeRefiner,new refiner.ENMergeDateRangeRefiner,new refiner.FRMergeDateRangeRefiner,new refiner.FRMergeDateTimeRefiner,new refiner.JPMergeDateRangeRefiner,new refiner.ExtractTimezoneOffsetRefiner,new refiner.ExtractTimezoneAbbrRefiner,new refiner.UnlikelyFormatFilter]}}var parser=require("./parsers/parser"),refiner=require("./refiners/refiner");exports.strictOption=function(){return baseOption(!0)},exports.casualOption=function(){var e=baseOption(!1);return e.parsers.unshift(new parser.ENCasualDateParser),e.parsers.unshift(new parser.ENWeekdayParser),e.parsers.unshift(new parser.JPCasualDateParser),e.parsers.unshift(new parser.ESCasualDateParser),e.parsers.unshift(new parser.ESWeekdayParser),e.parsers.unshift(new parser.FRCasualDateParser),e.parsers.unshift(new parser.FRWeekdayParser),e};
},{"./parsers/parser":37,"./refiners/refiner":48}],4:[function(require,module,exports){
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,PATTERN=/(\W|^)(now|today|tonight|last\s*night|(?:this|tomorrow|tmr|yesterday)\s*(morning|afternoon|evening)|tomorrow|tmr|yesterday)(?=\W|$)/i;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return PATTERN},this.extract=function(t,r,e,a){var t=e[0].substr(e[1].length),s=e.index+e[1].length,n=new ParsedResult({index:s,text:t,ref:r}),o=moment(r),i=o.clone(),m=t.toLowerCase();if("tonight"==m?(n.start.imply("hour",22),n.start.imply("meridiem",1)):/^tomorrow|^tmr/.test(m)?o.hour()>1&&i.add(1,"day"):/^yesterday/.test(m)?i.add(-1,"day"):m.match(/last\s*night/)?(n.start.imply("hour",0),o.hour()>6&&i.add(-1,"day")):m.match("now")&&(n.start.imply("hour",o.hour()),n.start.imply("minute",o.minute()),n.start.imply("second",o.second()),n.start.imply("millisecond",o.millisecond())),e[3]){var d=e[3].toLowerCase();"afternoon"==d?n.start.imply("hour",15):"evening"==d?n.start.imply("hour",18):"morning"==d&&n.start.imply("hour",6)}return n.start.assign("day",i.date()),n.start.assign("month",i.month()+1),n.start.assign("year",i.year()),n.tags.ENCasualDateParser=!0,n}};
...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,util=require("../../utils/FR"),DAYS_OFFSET=util.WEEKDAY_OFFSET,PATTERN=new RegExp("(\\W|^)(?:(Dimanche|Lundi|Mardi|mercredi|Jeudi|Vendredi|Samedi|Dim|Lun|Mar|Mer|Jeu|Ven|Sam)\\s*,?\\s*)?([0-9]{1,2})(?:\\s*(?:au|\\-|\\–|jusqu'au?|\\s)\\s*([0-9]{1,2})(?:er)?)?\\s*(?:de)?\\s*(Jan(?:vier|\\.)?|Fév(?:rier|\\.)?|Mars|Avr(?:il|\\.)?|Mai|Juin|Juil(?:let|\\.)?|Ao[uû]t|Sept(?:embre|\\.)?|Oct(?:obre|\\.)?|Nov(?:embre|\\.)?|déc(?:embre|\\.)?)(?:\\s*(\\s*[0-9]{1,4}(?![^\\s]\\d))(?:\\s*(AC|[ap]\\.?\\s*c(?:h(?:r)?)?\\.?\\s*n\\.?))?)?(?=\\W|$)","i"),WEEKDAY_GROUP=2,DATE_GROUP=3,DATE_TO_GROUP=4,MONTH_NAME_GROUP=5,YEAR_GROUP=6,YEAR_BE_GROUP=7;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return PATTERN},this.extract=function(e,t,r,a){var s=new ParsedResult({text:r[0].substr(r[1].length,r[0].length-r[1].length),index:r.index+r[1].length,ref:t}),n=r[MONTH_NAME_GROUP];n=util.MONTH_OFFSET[n.toLowerCase()];var i=r[DATE_GROUP];i=parseInt(i);var E=null;if(r[YEAR_GROUP]&&(E=r[YEAR_GROUP],E=parseInt(E),r[YEAR_BE_GROUP]?/a/i.test(r[YEAR_BE_GROUP])&&(E=-E):100>E&&(E+=2e3)),E)s.start.assign("day",i),s.start.assign("month",n),s.start.assign("year",E);else{var d=moment(t);d.month(n-1),d.date(i),d.year(moment(t).year());var u=d.clone().add(1,"y"),R=d.clone().add(-1,"y");Math.abs(u.diff(moment(t)))<Math.abs(d.diff(moment(t)))?d=u:Math.abs(R.diff(moment(t)))<Math.abs(d.diff(moment(t)))&&(d=R),s.start.assign("day",i),s.start.assign("month",n),s.start.imply("year",d.year())}if(r[WEEKDAY_GROUP]){var m=r[WEEKDAY_GROUP];m=util.WEEKDAY_OFFSET[m.toLowerCase()],s.start.assign("weekday",m)}return r[DATE_TO_GROUP]&&(s.end=s.start.clone(),s.end.assign("day",parseInt(r[DATE_TO_GROUP]))),s.tags.FRMonthNameLittleEndianParser=!0,s}};
},{"../../result":49,"../../utils/FR":52,"../parser":37,"moment":1}],26:[function(require,module,exports){
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,PATTERN=new RegExp("(\\W|^)(?:((?:dimanche|dim|lundi|lun|mardi|mar|mercredi|mer|jeudi|jeu|vendredi|ven|samedi|sam|le))\\s*\\,?\\s*)?([0-3]{0,1}[0-9]{1})[\\/\\.\\-]([0-3]{0,1}[0-9]{1})(?:[\\/\\.\\-]([0-9]{4}s*,?s*|[0-9]{2}s*,?s*))?(\\W|$)","i"),DAYS_OFFSET={dimanche:0,dim:0,lundi:1,lun:1,mardi:2,mar:2,mercredi:3,mer:3,jeudi:4,jeu:4,vendredi:5,ven:5,samedi:6,sam:6},OPENNING_GROUP=1,ENDING_GROUP=6,WEEKDAY_GROUP=2,DAY_GROUP=3,MONTH_GROUP=4,YEAR_GROUP=5;exports.Parser=function(e){Parser.apply(this,arguments),this.pattern=function(){return PATTERN},this.extract=function(e,r,a,t){if("/"==a[OPENNING_GROUP]||"/"==a[ENDING_GROUP])return void(a.index+=a[0].length);var s=a.index+a[OPENNING_GROUP].length,e=a[0].substr(a[OPENNING_GROUP].length,a[0].length-a[ENDING_GROUP].length),n=new ParsedResult({text:e,index:s,ref:r});if(!e.match(/^\d\.\d$/)&&!e.match(/^\d\.\d{1,2}\.\d{1,2}$/)&&(a[YEAR_GROUP]||!(a[0].indexOf("/")<0))){var i=a[YEAR_GROUP]||moment(r).year()+"",d=a[MONTH_GROUP],P=a[DAY_GROUP];if(P=parseInt(P),d=parseInt(d),i=parseInt(i),(1>d||d>12)&&d>12){if(!(P>=1&&12>=P&&d>=13&&31>=d))return null;var R=d;d=P,P=R}return 1>P||P>31?null:(100>i&&(i+=i>50?1900:2e3),n.start.assign("day",P),n.start.assign("month",d),n.start.assign("year",i),a[WEEKDAY_GROUP]&&n.start.assign("weekday",DAYS_OFFSET[a[WEEKDAY_GROUP].toLowerCase()]),n.tags.FRSlashDateFormatParser=!0,n)}}};
